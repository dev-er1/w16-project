...
Отдельный workspace `example-langs` содержит экспериментальные языки, которые используют W16 как backend:

```text
example-langs/w16-cc - экспериментальный C frontend, сокращённо WCC
```
...
Например, `example-langs/w16-cc` разбирает C-подобный код:
...
9. `example-langs/w16-cc/src/codegen/` — пример frontend-а, который генерирует W16 HIR.
