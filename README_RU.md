...
Этот проект находится под лицензией [Apache 2.0](LICENSE).

*Скоро у меня будут экзамены, поэтому проект может не обновлятся.*

# W16
W16 — runtime с многоуровневым IR pipeline (HIR -> MIR -> Bytecode) с поддержкой интерпретации и JIT-компиляции.

## Статус

- HIR: Стабильный
- MIR: Стабильный
- Bytecode VM: Стабильный
- Interpreter: Стабильный
- JIT: Стабильный
- AOT: Экспериментальный.

## CLI
```text
- run <file>     — Запустить программу через                .
  - -i                                      — Интерпретацию
  - -j                                      — JIT-компиляцию
  - --time       — Показать время выполнения кода.
...
- build <file>   — Скомпилировать программу (через AOT-компиляцию, нестабильно)
- version        — Версия CLI(именно CLI, не runtime-а)
```
...
*Бэкенд <u>ПОЛНОСТЬЮ</u> полагается на фронтенд, ноль семантики на уровне байт-кода.*
...
Cranelift IR, и машинный код).[Перечисление байт-кода](w16-core\src\bytecode.rs)
