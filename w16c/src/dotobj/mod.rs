//! # Генератор объектного файла.
//!
//! Данный модуль реализует генерацию объектного файла, для последующей компиляции в
//! исполняемый файл.
//!
//! ### Архитектурный переход от AOT кAOT:
//! Логика трансляции инструкций, отработанная в AOT-компиляторе, остается неизменной,
//! так как генерация промежуточного представления (IR) идентична. Основное различие
//! заключается в управлении модулем:
//!
//! * **Модуль**: `cranelift_AOT::AOTModule` заменен на `cranelift_object::ObjectModule`.
//!   Это позволяет не исполнять код в памяти, а формировать структуру объектного файла.
//! * **Целевая платформа**: В отличие от AOT, использующего хостовую ISA,AOT позволяет
//!   генерировать код под любую платформу (Cross-compilation) через `target-lexicon`.
//! * **Финализация**: Вместо получения указателя на функцию в памяти, модуль завершается
//!   экспортом сырых байтов (секции кода, данных и таблицы релокаций), которые
//!   сохраняются на диск.
//! * **Линковка**: Сгенерированный файл является "полуфабрикатом". Для получения
//!   исполняемого бинарника модуль инициирует вызов системного линковщика (ld, link.exe
//!   или lld), связывая код с необходимыми внешними символами runtime.
//!
//! ### Зачем нужен объектный файл:
//! Объектный файл (.obj / .o)  — это промежуточная форма программы, в которой
//! высокоуровневый байт-код W16 уже превращен в машинные инструкции целевого
//! процессора, но еще не привязан к конкретным адресам в памяти.
//!
//! Это позволяет:
//! 1. **Статическая связь**: Объединять код W16 с системными библиотеками (CRT, WinAPI).
//! 2. **Кросс-платформенность**: Генерировать файлы для Windows, находясь на Linux,
//!    просто меняя настройки `ObjectBuilder`.
//! 3. **Оптимизация**: Дать линковщику возможность провести финальную оптимизацию
//!    (LTO) на этапе сборки исполняемого файла.
//!
//! ### Что здесь происходит:
//! В данном модуле `cranelift-object` выступает в роли "упаковщика".
//! После того как `FunctionBuilder` заканчивает перевод байт-кода в IR,
//! этот модуль:
//! - Определяет формат файла (ELF, COFF, PE) через `target-lexicon`.
//! - Создает стандартные секции кода и данных.
//! - Генерирует таблицу символов, экспортируя функции W16 для внешнего мира.
//! - Создает релокации для всех внешних вызовов (например, обращения к runtime-функциям вывода).
//!
//! Да, можно сказать что я просто взял код AOT-компилятора. Но так то логика AOT-компилятора верна,
//! главное просто нужно было поменять cranelift_AOT::AOTModule на cranelift_object::ObjectModule для генерации объектного файла,
//! и добавить функции для сохранения файла, и вызов системного линковщика, ну а ещё изменить финализацию
//! чтобы получить массив байтов для объектного файла.
use std::collections::BTreeSet;
use std::fmt;

use cranelift::codegen::ir::BlockArg;
use cranelift::prelude::*;
use cranelift_codegen::{Context, ir::Value};
use cranelift_codegen::{
    ir::{
        AbiParam, Block, InstBuilder, MemFlags, Type,
...
impl fmt::Display for AOTError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            AOTError::UnsupportedOpcode { ip, opcode } => {
                write!(f, "AOT does not support {opcode:?} at ip {ip}")
            }
            AOTError::ConstantPoolOutOfBounds { ip, offset } => {
                write!(
                    f,
                    "LoadConst at ip {ip} reads past constant pool at offset {offset}"
                )
            }
            AOTError::DynamicJumpTarget { ip, register } => {
                write!(
                    f,
                    "jump at ip {ip} uses non-constant target register r{register}"
                )
            }
            AOTError::InvalidRegisterJump { ip, target } => {
                write!(f, "jump at ip {ip} targets invalid instruction {target}")
            }
            AOTError::Cranelift(message) => write!(f, "Cranelift error: {message}"),
        }
    }
}
...
impl AOT {
    ...
    pub fn new(target: Triple, opt_level: OptLevel, module_name: &str) -> Self {
        ...
        let isa = cranelift::codegen::isa::lookup(target)
            .unwrap_or_else(|msg| panic!("Target ISA not supported: {msg}"))
            .finish(flags)
            .unwrap();
    ...
    /// Скомпилировать W16 байткод в объектный файл.
    ///
    /// При ошибке компиляции вызывает `panic`. Для обработки ошибок используйте
    /// `try_compile`.
    pub fn compile(self, bytecode: &Bytecode, output_path: &str) -> String {
        self.try_compile(bytecode, output_path)
            .unwrap_or_else(|error| panic!("Building of object file failed: {error}"))
    }

    /// Попытаться скомпилировать W16 байткод в объектный файл.
    ///
    /// Возвращает имя сгенерированного объектного файла при успехе или `AOTError`,
    ...
    pub fn try_compile(
        ...
        let obj_bytes = self.emit_object_file()?;
    ...
    fn declare_imports(&mut self, int_ptr: Type) -> Result<Imports, AOTError> {
        ...
        let frem = self
            .module
            .declare_function("w16_object_frem", Linkage::Import, &frem_sig)
            .map_err(|error| AOTError::Cranelift(error.to_string()))?;
        let print_int = self
            .module
            .declare_function("w16_object_print_int", Linkage::Import, &print_one_sig)
            .map_err(|error| AOTError::Cranelift(error.to_string()))?;
        let print_uint = self
            .module
            .declare_function("w16_object_print_uint", Linkage::Import, &print_one_sig)
            .map_err(|error| AOTError::Cranelift(error.to_string()))?;
        let print_float = self
            .module
            .declare_function("w16_object_print_float", Linkage::Import, &print_one_sig)
            .map_err(|error| AOTError::Cranelift(error.to_string()))?;
        let print_str = self
            .module
            .declare_function("w16_object_print_str", Linkage::Import, &print_str_sig)
            .map_err(|error| AOTError::Cranelift(error.to_string()))?;
    ...
    /// Динамические цели переходов отклоняются с `AOTError::DynamicJumpTarget`.
    fn analyze_jumps(&self, bytecode: &Bytecode) -> Result<JumpTargets, AOTError> {
    ...
    /// Завершает сборку и возвращает сырые байты готового объектного файла.
    pub fn emit_object_file(self) -> Result<Vec<u8>, AOTError> {
...
/// полностью, как `u64`.
fn validate_constants(bytecode: &Bytecode) -> Result<(), AOTError> {
    for (ip, instr) in bytecode.instructions.iter().enumerate() {
        if instr.opcode == OpCode::LoadConst {
            let offset = instr.imm16() as usize;
            if offset + 8 > bytecode.constant_pool.data.len() {
                return Err(AOTError::ConstantPoolOutOfBounds { ip, offset });
            }
        }
    }
    Ok(())
...
fn resolve_target(
    ...
    let target =
        const_regs[register as usize].ok_or(AOTError::DynamicJumpTarget { ip, register })? as usize;
    if target > isize::MAX as usize {
        return Err(AOTError::InvalidRegisterJump { ip, target });
    }
...
fn update_const_regs(const_regs: &mut [Option<u64>; REGISTER_COUNT], instr: &Instruction) {
